var ProfesorComposite = Marionette.CompositeView.extend({
   ...
   // var miProfesorCollection = new ProfesorCollection(JSON.parse(localStorage.getItem('estudianteCollection')));
   // var miProfesor = miProfesorCollection.where({cedula: $cedula.val()})[0];

   // miEstudiante.collection.set({
    //  nombre : $nombre.val(),
    //  cedula : $cedula.val(),
     // salario : $salario.val(),
    //  diasLaborales : $dias.val(),
